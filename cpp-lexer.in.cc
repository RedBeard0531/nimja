...
#include "../cpp-lexer.in.h"

#include <stdio.h>

using namespace std;

bool Lexer::Error(const char* message) {
  // Compute line/column.
  int line = 1;
...
const char* Lexer::DescribeLastError() {
  if (last_token_) {
...
// These are in lexer.nim, but there isn't a great way to generate a header.
// Name mangling should help catch if this is incorrect.
void AddText(EvalString*, StringPiece);
void AddSpecial(EvalString*, StringPiece);

bool Lexer::ReadEvalString(EvalString* eval, bool path) {
  const char* p = ofs_;
  const char* q;
  const char* start;
  for (;;) {
    ...
    /*!re2c
    [^$ :\r\n|\000]+ {
      AddText(eval, StringPiece(start, p - start));
      continue;
    }
    "\r\n" {
      if (path)
    ...
    [ :|\n] {
      if (path) {
        ...
        if (*start == '\n')
          break;
        AddText(eval, StringPiece(start, 1));
        continue;
      }
    }
    "$$" {
      AddText(eval, StringPiece("$", 1));
      continue;
    }
    "$ " {
      AddText(eval, StringPiece(" ", 1));
    ...
    "${"varname"}" {
      AddSpecial(eval, StringPiece(start + 2, p - start - 3));
      continue;
    }
    "$"simple_varname {
      AddSpecial(eval, StringPiece(start + 1, p - start - 1));
      continue;
    }
    "$:" {
      AddText(eval, StringPiece(":", 1));
    ...
    "$". {
      last_token_ = start;
      return Error("bad $-escape (literal $ must be written as $$)");
    }
    nul {
      last_token_ = start;
    ...
    [^] {
      last_token_ = start;
      return Error(DescribeLastError());
    }
    */
  }
  last_token_ = start;
